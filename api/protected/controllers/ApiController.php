...
class ApiController extends Controller {
    ...
    public function beforeAction() {
        ...
        try { 
            ...
            if (($GIZURCLOUD_SECRET_KEY = Yii::cache->get($_SERVER['HTTP_X_GIZURCLOUD_API_KEY']))===false) {
                ...
                if ($publicKeyNotFound = ($ddb_reponse->body->count==0)) {
                    //Scan for API KEYS
                    $ddb_response = $dynamodb->scan(array(
                        'TableName'       => $table_name,
                    ...
                    if (!($publicKeyNotFound = ($ddb_reponse->body->count==0)))  
                        $GIZURCLOUD_SECRET_KEY = (string)$ddb_response->body->Items->apikey_2->{AmazonDynamoDB::TYPE_STRING};
                } else {
                    $GIZURCLOUD_SECRET_KEY = (string)$ddb_response->body->Items->apikey_1->{AmazonDynamoDB::TYPE_STRING};
                }
