...
/**
 * 2012-07-17, Jonas Colmsjö, Gizur AB
 *
 *
 * The purpose of this wrapper is to make is possible to share the vTiger code base 
 * between vtiger instances (still having separate databases)
 *
 * Do some setup of the vtiger in order to create a wrapper:
 * - Add a new include file containing some paths etc.
 * - Change current directory to vTiger installation
 * - Add the wrapper directory to the front of the PHP include path in order to replace config.inc.php
 * - Comment out the part that redirect to install.php of config.inc.php does not exist in the current directory
 *   (needed since the file has been moved to the wrapper)
 *
 * Coding standards:
...
// get current value of PHP include_path
$current_inc_path = get_include_path();

// add directory with config.inc.php to the include path
set_include_path($current_inc_path . ':' . VTWRAPPER_PATH);

vtwrapper_debug( get_include_path() );

// Allow this file to be included using XMLHttpRequest
header('Access-Control: allow <' . VTWRAPPER_DOMAIN . '>');
...
// vtwrapper, disable check for config.inc.php since it is stored elsewere now
