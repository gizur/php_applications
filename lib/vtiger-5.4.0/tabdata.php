...
$ddb_response = $dynamodb->get_item(
    array(
        ...
        'Key' => $dynamodb->attributes(
            array(
                'HashKeyElement' => $gizur_client_id,
            )
        ),
        'ConsistentRead' => 'true'
    )
...
if (isset($ddb_response->body->Item)) {
    $items = $ddb_response->body->Item->children();
    $tab_info_array = $items[0]['tab_info_array'];
    $tab_seq_array = $items[0]['tab_seq_array'];
    $tab_ownedby_array = $items[0]['tab_ownedby_array'];
    $action_id_array = $items[0]['action_id_array'];
    $action_name_array = $items[0]['action_name_array'];
}else{*/
