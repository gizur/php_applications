...
function getblock_header($str, $headerspan = '4', $ticketcloselink = false)
{
    ...
    if ($ticketcloselink == true) {
        $list .='<tr><td colspan="' . ($headerspan - 1) . 
            '" class="detailedViewHeader"><b>' . getTranslatedString($str) . 
            '</b></td>';
        $list .='<td class="detailedViewHeader"><div align="right">' . 
            $ticketcloselink . '</div></td></tr>';
    } else {
        $_SESSION['block'] = $str;
        $list .='<tr><td colspan="' . ($headerspan) . 
            '" class="detailedViewHeader"><b>' . 
            getTranslatedString($str) . '</b></td></tr>';
    }
    return $list;
}

/* Commented By Anil Singh this is org function
...
//////////////////////////// By Anil Singh //////////////////// 
//Old function is getblock_fieldlist1 (Above commented)

function getblock_fieldlist($block_array, $blockNameDisable = "")
{
    $list = '';
    $z = 0;
    ///////////////////////////////  Added By Anil Singh
    foreach ($block_array as $newarr) {
        $block_array1[] = $newarr;
    }
    $field_count = count($block_array1);
    /////////////////////////////////////// End /////////////////////////////
    ////// Remove Block //////////////////////// Added By Anil Singh
    if (!empty($blockNameDisable)) {
        foreach ($blockNameDisable as $RmBlock) {
            $base64_remome_block[] = base64_encode($RmBlock);
        }
    }
    //////////////////////////////////////////////////////////////  End
    if ($field_count != 0) {
        for ($i = 0; $i < $field_count; $i++, $z++) {
            $blockname = $block_array1[$i]['blockname'];
            //// Added By Anil Singh
            if (!@in_array(base64_encode($blockname), $base64_remome_block)) { 
                $data = $block_array1[$i]['fieldvalue'];
                if ($block_array1[$i]['fieldlabel'] == 'Note') {
                    $data = html_entity_decode($data);
                }
                //////////////////  Added By Anil Singh  for cospan description
                if ($block_array1[$i]['fieldlabel'] == 'Description') {
                    $desfieldspan = "colspan=3";
                } else {
                    $desfieldspan = "";
                };
                ///////////////////////////
                if ($data == '')
                    $data = '&nbsp;';
                if (strcmp($blockname, $block_array1[$i - 1]['blockname'])) {
                    //hardcoded for tickets information block so 
                ...
                if ($z == 0 || $z % 2 == 0)
                    $list .= '<tr>';
                $list .= '<td class="dvtCellLabel" width="20%" align="right">' .
                    getTranslatedString($block_array1[$i][fieldlabel]) . '</td>';
                if (($z == 0 || $z % 2 == 0) && ($i == ($field_count - 1)))
                    $list .= '<td colspan="3" width="20%" class="dvtCellInfo">' .
                    getTranslatedString($data) . '</td>';
                else
                    $list .= '<td width="20%" class="dvtCellInfo" ' . 
                    $desfieldspan . '>' . getTranslatedString($data) . '</td>';
                if ($z % 2 == 1 || ($i == ($field_count - 1) ))
                    $list .= '</tr>';
            }
        } //////// End block if		
...
//only for product
function getblock_fieldlistview_product($block_array1, $module, $rmfieldsarr = "")
{
    ...
    if ($block_array1 == '') {
        $list.='<tr><td>' . $module . " " . 
            getTranslatedString('LBL_NOT_AVAILABLE') . '</td></tr>';
        return $list;
    }

//// Old Code for($k=0;$k<=2;$k++) /// By Anil Singh
    for ($k = 0; $k < 2; $k++) {
/////////////////////////////////////////////////////////// Added By Anil Singh
        if (!empty($block_array1[$k][$module]['head'][0])) {
            foreach ($block_array1[$k][$module]['head'][0] as $val) {
                $block_array[$k][$module]['head'][0][] = $val;
            }
        }
        $header_arr = $block_array[$k][$module]['head'][0];
        $nooffields = count($header_arr);
        if (!empty($block_array1[$k][$module]['data'])) {
            foreach ($block_array1[$k][$module]['data'] as &$valdata) {
                $valdata = array_values($valdata);
            }
        }
        $data_arr = $block_array1[$k][$module]['data'];
...
////////////////////////////////////////////////////////////////////////// End
        ...
        if ($nooffields != '') {
            ...
            for ($i = 0; $i < $nooffields; $i++)
                $list .= "<td>" . 
                getTranslatedString($header_arr[$i]['fielddata']) . "</td>";
            $list .='</tr>';
        }
        if ($noofdata != '') {
...
function getblock_fieldlistview($block_array, $block)
{
    if ($block_array[0] == "#MODULE INACTIVE#") {
        $list.='<tr><td>' . getTranslatedString($block) . " " . 
            getTranslatedString('MODULE_INACTIVE') . '</td></tr>';
        return $list;
    }
    if ($block_array == '') {
        $list.='<tr><td>' . getTranslatedString($block) . " " . 
    ...
    if ($nooffields != '') {
        ...
        for ($i = 0; $i < $nooffields; $i++)
            $list .= "<td  class='detailedViewHeader' >" . 
            getTranslatedString($header_arr[$i]['fielddata']) . "</td>";
        $list .='</tr>';
    }
    if ($noofdata != '') {
...
//////////////////////////// This function to be Added by Anil Singh @25 Dec2011
function remove_item_by_value(&$array, $val = '', $lavel = '', $preserve_keys = true)
{
    $newarray = "";
    $val = strtoupper($val);
    for ($i = 0; $i < count($array); $i++) {
        $newarray[] = strtoupper($array[$i][$lavel]);
        $key = array_search($val, $newarray);
    }
    return $key; //($preserve_keys === true) ? $array : array_values($array);
}

function array_remove_key($arr)
{
